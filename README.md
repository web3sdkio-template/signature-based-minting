...
```bash
npx web3sdkio create --template signature-based-minting
```

- Create your own NFT Collection via the web3sdkio dashboard (follow the steps in **Setting Up The NFT Collection** of this doc if you need more help)
...
To create an NFT collection, we can use the web3sdkio dashboard and create a fully customizable NFT collection in just a few clicks.

Head to https://web3sdk.io/dashboard and create a new contract on the Polygon Mumbai network. Click **Create NFTs and Tokens** > **NFT Collection**.

Give your collection a name and click **Deploy now**!

Nice! Now we have an NFT collection. Let's set up a project and see how we can start minting some NFTs using the web3sdkio SDK.
...
Over at the home page at `index.tsx`, we're using the [web3sdkio React SDK MetaMask Connector](https://docs.web3sdk.io/react/category/wallet-connection) so our user can connect their wallet to our website.
...
For our community collection, each NFT is minted by a different user. We're using another web3sdkio hook called `useNFTCollection` to connect to our smart contract and load all of the NFT's in our collection.

```tsx
// Fetch the NFT collection from web3sdkio via it's contract address.
...
**Initialize the Web3sdkio SDK on the server-side**

```tsx
// Initialize the Web3sdkio SDK on the serverside
